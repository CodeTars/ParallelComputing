 ...
 *   |An-1,k+1 ... An-1,n|
 * 
 * n(gridDim.y) * ceil((n + 1) / EliminationBlockDim_X)(gridDim.x) blocks per Grid
...
{
    int i = blockIdx.y; // threadIdx.y 恒为0
...
int main(int argc, char* argv[])
{
     ...
     * */
    const dim3 GRID_DIM(ceil(1.0 * (n + 1) / EliminationBlockDim_X), n, 1);
    const dim3 BLOCK_DIM(EliminationBlockDim_X, 1, 1);
    ...
    /* 输出 */
    Output(A, n);
    PrintTime("gauss2.3", n, malloc_time, copy_time, kernel_time1, kernel_time2, kernel_time3, backSub_time);
